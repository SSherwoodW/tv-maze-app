...
async function getShowsByTerm(search) {
  ...
  let res = await axios.get(`http://api.tvmaze.com/search/shows?q=${search}`)
  console.log(res);

  return res.data.map(result => {
    const show = result.show; //don't understand this arrow function.
    return {
      id: show.id,
      name: show.name,
...
async function getEpisodesAndDisplay(evt){
  const showId = $(evt.target).closest('.Show').data('data-show-id');
